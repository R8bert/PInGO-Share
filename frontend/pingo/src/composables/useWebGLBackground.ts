...
export function useWebGLBackground(options: WebGLBackgroundOptions = {}) {
  ...
  function setupWebGL() {
    ...
    gl = (canvasRef.value.getContext('webgl') || canvasRef.value.getContext('experimental-webgl')) as WebGLRenderingContext
    if (!gl) {
  ...
  function render() {
    ...
    // Resize canvas
    const displayWidth = parent.clientWidth * resolutionScale
    const displayHeight = parent.clientHeight * resolutionScale
    ...
    const scanLocation = gl.getUniformLocation(program, 'uScan')
    const scanFreqLocation = gl.getUniformLocation(program, 'uScanFreq')
    ...
    gl.uniform1f(scanLocation, scanlineIntensity)
    gl.uniform1f(scanFreqLocation, scanlineFrequency)
