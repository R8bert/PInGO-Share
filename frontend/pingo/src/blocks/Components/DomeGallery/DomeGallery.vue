...
<template>
  ...
  >
    <main
      ref="mainRef"
      class="absolute inset-0 grid place-items-center overflow-visible touch-none select-none bg-transparent"
    >
      <div
...
const autoRotateStep = (now: number) => {
  ...
  if (canSpin && deltaMs > 0) {
    const nextY = wrapAngleSigned(
      rotation.value.y + deltaMs * AUTO_ROTATE_SPEED_DEG_PER_MS,
    );
...
const onDragMove = (e: MouseEvent | TouchEvent) => {
  ...
  const nextY = wrapAngleSigned(
    startRotation.value.y + dxTotal / props.dragSensitivity,
  );
...
const onDragEnd = (e: MouseEvent | TouchEvent) => {
  ...
  if (hasMoved.value && startPosition.value) {
    ...
    const clientY =
      "touches" in e ? (e.changedTouches?.[0]?.clientY ?? 0) : e.clientY;
...
onMounted(() => {
  ...
  resizeObserver = new ResizeObserver((entries) => {
    ...
    const overlay = viewerRef.value?.querySelector(
      ".enlarge",
    ) as HTMLElement | null;
